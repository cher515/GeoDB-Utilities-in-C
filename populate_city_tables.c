...
int main(int argc, char* argv[])
{
   ...
   sql = (char *) "INSERT INTO `city` VALUES (1,'Kabul','AFG','Kabol',1780000);" \
...
"INSERT INTO `city` VALUES (4079,'Rafah','PSE','Rafah',92020);" ; 
   ...
   if( rc != SQLITE_OK ){
      fprintf(stderr, "SQL error when instering data into city table: %s\n", zErrMsg);
      sqlite3_free(zErrMsg);
   }else{
      fprintf(stdout, "Table city records created successfully\n");
   }



   sql = (char *) "INSERT INTO `country` VALUES ('ABW','Aruba','North America','Caribbean',193.00,NULL,103000,78.4,828.00,793.00,'Aruba','Nonmetropolitan Territory of The Netherlands','Beatrix',129,'AW');" \
...
"INSERT INTO `country` VALUES ('ZWE','Zimbabwe','Africa','Eastern Africa',390757.00,1980,11669000,37.8,5951.00,8670.00,'Zimbabwe','Republic','Robert G. Mugabe',4068,'ZW');" ;
   ...
   rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);
   if( rc != SQLITE_OK ){
      fprintf(stderr, "SQL error when insterting data into country table: %s\n", zErrMsg);
      sqlite3_free(zErrMsg);
   }else{
      fprintf(stdout, "Table country records created successfully\n");
   }

    sql = (char *)"INSERT INTO `countrylanguage` VALUES ('ABW','Dutch','T',5.3);" \
...
"INSERT INTO `countrylanguage` VALUES ('ZWE','Shona','F',72.1);" ;
   ...
   rc = sqlite3_exec(db, sql, callback, 0, &zErrMsg);
   if( rc != SQLITE_OK ){
      fprintf(stderr, "SQL error when insterting data in countrylanguage table: %s\n", zErrMsg);
      sqlite3_free(zErrMsg);
   }else{
      fprintf(stdout, "Table countrylanguage records created successfully\n");
   }
